'use strict' //独自のメソッドを追記しているので互換性がありません！20231011


//サンプル

function test_getSheet() {
  const postSheetName = "ポスト管理"
  const postSheet = new Sheet(SpreadsheetApp.getActiveSpreadsheet().getSheetByName(postSheetName))
  const sheetDitcs = postSheet.getAsDicts()

  console.log(sheetDitcs)
...
class Sheet {
   ...
   * @param {number} headerRows - ヘッダーの行数(デフォルトは1)
   * @param {number} headerIndex - ヘッダー行のインデックス (ユニークなカラム)デフォルトは0
   */
  ...
  hideSheet() { return this.sheet.hideSheet(); }
  showSheet() { return this.sheet.showSheet(); }
  ...
  /**
   * getAsDictsで取得できるようなMap形式配列をシートに追加するメソッド
   * @param {Array.<Map>} - dicts
   * @return {this}
   */
  appendDicts(dicts) {
  ...
  /**
   * 対象列をキーとして値があれば更新。なければレコードを追加するメソッド
   * @param {string} headerName - ヘッダー名（プライマリーキーとして機能）
   * @param {Array<Map>} newDicts - 更新または追加するデータの配列
   * @return {Object} this - メソッドチェーン用にインスタンス自体を返す
   */
  upsertDicts(headerName, newDicts) {
    const dicts = this.getAsDicts();
    const idToRow = new Map();

    dicts.forEach((dict, i) => {
      idToRow.set(dict.get(headerName), i);
    });

    newDicts.forEach(dict => {
      const value = dict.get(headerName);
      if (idToRow.has(value)) {
        const index = idToRow.get(value);
        this.updateDicts(index, dict);
      } else {
        this.dicts_.push(dict);
      }
    });

    this.setDictsHeaderRowsAfter(this.dicts_);
    return this;
  }

  /**
   * 対象列をキーとして、新しいものだけレコードを追加するメソッド（更新はしない）
   * @param {string} headerName - ヘッダー名（プライマリーキーとして機能）
   * @param {Array<Map>} dicts - 追加するデータの配列
   * @param {string} [timestampHeaderName=""] - タイムスタンプを保存するヘッダー名
   * @param {Date} [timestamp=new Date()] - タイムスタンプ
   * @return {Object} this - メソッドチェーン用にインスタンス自体を返す
   */
  insertNewDicts(headerName, dicts, timestampHeaderName = "", timestamp = new Date()) {
    dicts.forEach(dict => {
      const index = this.findDictIndex(headerName, dict);
      if (index === -1) {
        if (timestampHeaderName !== "") {
          dict.set(timestampHeaderName, timestamp);
        }
        this.dicts_.push(dict);
      }
    });
  ...
  /**
   * レコードをすべて削除し、値(Dicts)を貼り付けるメソッド
   * @param {Array.<Map>} Dicts - 貼り付ける値
   */
  setDictsHeaderRowsAfter(dicts) {
    this.clearDataValues();
    if (dicts.length === 0) return this;
    this.appendDicts(dicts);
    return this;
  }

  /**
   * 対象のindexのdictsを更新するメソッド
   * @param {number} index - index
   * @param {Map} Map - Map
   * @return {array.<Map>} dicts - dicts
   */
  updateDicts(index, dict) {
    const dicts = this.getAsDicts();
    dict.forEach((record, headerName) => {
      dicts[index].set(headerName, record);
    })
    this.dicts_ = dicts;
    return dicts;
  }
  ...
  /**
   * 抽出対象の列の一番最初に合致した index を取得するメソッド
   * @param {string} headerName - 抽出対象の列のヘッダー名
   * @param {string|number|boolean|Date} dict - 抽出対象の値
   * @return {number} - index
   */
  findDictIndex(headerName, dict) {

    const dicts = this.getAsDicts();
    const index = dicts.findIndex(record => {
      return record.get(headerName) === dict.get(headerName);
    })
    return index;
  }
  ...
  /**
   * 
   * 対象の dict が 列に含まれるかどうかを調べるメソッド
   * @param {Map} dict - dict
   * @param {string} key - key
   */
  hasDict(dict, headerName) {
    const dicts = this.getAsDicts();

    return dicts.some(record => {
      return record.get(headerName) === dict.get(headerName);
    })
  }
